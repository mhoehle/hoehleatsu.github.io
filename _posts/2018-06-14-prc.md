...
the document contains a large body of references containing the
details of the calculations - in particular @kagan_etal2008a and
...
[German Gene Diagnostic Commission](https://www.rki.de/DE/Content/Kommissionen/GendiagnostikKommission/Richtlinien/RL-VorgeburtlicheRisikoabklaerung.pdf?__blob=publicationFile)
...
```
## Warning in file(file, "rt"): cannot open file '/Users/hoehle/Sandbox/Blog/figure/source/
## 2018-06-11-prc/merzetal2015-tab5.csv': No such file or directory
```
...
```
## Error in eval(lhs, parent, parent): object 'tab5' not found
```

```
## Error in grep("our_", names(tab5)): object 'tab5' not found
```
...
Altogether, we observe that the results do not fully agree, but
  are in most cases of the same magnitudee. Even when using the same prior
...
comprehensive as demanded by the Gene Diagnostic Commission? Well,
...
been a start. Even better would be an open-source reference implementation! To this data it remained
