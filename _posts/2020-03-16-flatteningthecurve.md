...
\right.
$$
where $\beta_0$ is the ordinary $\beta$ value of the disease. We will use a drastic reduction of the contacts within the interval $[t_1, t_2]$ and thus $\beta_1 < \beta_0$. After some time, the control measures are reduced slightly, i.e. $\beta_1 < \beta_2 < \beta_0$.
...
Here we used the rather conservative estimate that $R_0$ can be reduced by 60% to 1.35 for a few weeks, after the reduction is
80% of the original $R_0$, i.e. 1.8. Things of course become more optimistic, the larger the reduction is. One trap is though to reduce $R_0$ drastically and then lift the measures too much - in this case the outbreak is delayed, but then almost of the same peak and size, only later.
