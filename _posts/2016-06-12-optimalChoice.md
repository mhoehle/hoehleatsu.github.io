...
[comment]: # https://openclipart.org/detail/171299/indecisive-silhouettesvg
[comment]: # ![](figs/indecisive-silhouette-300px-scaled.png)
![]({{ site.url }}/figure/source/2016-06-12-optimalChoice/indecisive-silhouette-300px-scaled.png "Source: https://openclipart.org/detail/171299/indecisive-silhouettesvg")
<br>
...
It is obvious that $\phi_n(1)=1/n$. When $n=10$, the remaining probabilities can
...
![plot of chunk unnamed-chunk-4](http://hoehleatsu.github.io/figure/source/2016-06-12-optimalChoice/unnamed-chunk-4-1.png)
...
In the example we thus actually manage to select the best flat! However, this is not always guaranteed: for example, if the best overall candidate is among the training sample (4/10 chance for this to happen) we would end up with the last candidate flat no matter how good or bad it is. As stated above: the probability that the above decision strategy will pick the best candidate is $\phi_{10}(4)=0.40$.
...
For small $n$ the optimal $r$ and corresponding probability of success can easily be computed numerically. However, for large $n$ the numerical precision as well as the computations become more tedious and hence interest is in finding a general asymptotic approximation as $n$ grows large: One can show that as $n$ gets large the optimal procedure is always to screen the first $1/e$ = 37% and then select the first candidate better than the training sample [@gilbert_mosteller1966]. The asymptotic probability of success, i.e. finding the overall best candidate, when following the such a procedure is also about 37%. Below we show a small table illustrating the precision of the asymptotic approximation.
...
![]({{ site.url }}/downloads/animation-s.gif)
![](animation-select.gif)
...
$$
c_{n-1} = E\left(\frac{n+1}{n+1}y_n\right) = \frac{1}{n} \sum_{j=1}^n = \frac{n+1}{2}.
$$
...
$$
where $[x]$ denotes the largest integer smaller or equal to $x$. Because at each stage $i$ we select between the expected payoff, if we decide to wait deciding for at least another round (=$c_i$), and the expected payoff of stopping now. Our optimal stopping time is thus

$$
\min_{i\in \{1,\ldots,n\}} \{ E(x_i|y_i=j) \geq c_{i} \}.
$$

Implicitly, the sequence of $s_i$'s contains the decision strategy. We transfer the procedure
...
```
In the above, the first element of the vectors `s` and `c` is the element 0. Hence, $s_1$ is located at position two of the vector. It's interesting to observe that for the example one forms a baseline for the same amount of time, but after a while becomes more 'desperate' and accepts candidates who are not optimal.
...
From the results it becomes clear that the expected rank optimizing function on average takes a little less time to decide. Furthermore, the expected rank is somewhat better than for the overall best decision strategy. We can also compare `sim["erank","rank"]`=2.562  against $c_0$=2.558.

# Discussion

Is the blog title "Mathematical advice for real life" an oxymoron?
No. Assumptions 1-6 clearly state the abstraction. You may not agree with this, but given that framework, the two functions `strategy_best`and
...
non-trivial -- even for mathematicians :smirk:.

<img src="http://hoehleatsu.github.io/figure/source/2016-06-12-optimalChoice/unnamed-chunk-14-1.png" title="plot of chunk unnamed-chunk-14" alt="plot of chunk unnamed-chunk-14" style="display: block; margin: auto;" />
