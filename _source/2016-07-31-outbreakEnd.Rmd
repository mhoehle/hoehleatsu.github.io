...
As a consequence, this blog entry is my attempt of a **repro-session** in connection with the IBC: The talk entitled *[Determining the end of an epidemic with human-to-human transmission](https://biometricconference.org/contributed-sessions/oral/detail/?sessionID=CS.15)* by [Hiroshi Nishiura](http://plaza.umin.ac.jp/~infepi/hnishiura.htm) was both interesting, from a field I'm interested in (infectious disease epidemiology) and the method he presented looked like it could be re-implemented in finite time. The methodological proposal communicated in @nishiura_etal2016 is about at which time point one can declare an outbreak of a communicable disease as having ended. Such statements are important in order to calm the population, tourists or to change your alertness status. The current WHO method to assess this currently requires that a period of two times the longest possible incubation time needs to have passed since the last cases before an outbreak can be declared as being over. However, as stated in their paper, the criterion clearly lacks statistical motivation. As an improvement the authors formulate a statistical criterion based on the serial interval distribution and the offspring distribution.

In what follows we shall quickly describe the method and apply it to the 2015 MERS-CoV outbreak in Korea. As a small outlook, we shall implement some own thoughts on how to answer the posed questions using a hierarchical model.

# Method

Given the currently available outbreak data about the symptom onset $D=\{t_i; i=1,\ldots,n\}$ in the $n$ cases of the outbreak, let $Y_t$ be a count variable representing the number of cases we will observe on a given day $t, t\geq \max_{i=1,\ldots,n}(t_i)$. In particular we will be interested in whether we will observe zero or more cases, that is we let $Z_t = I(Y_t>0)$. Furthermore, let $F_{\text{serial}}$ be the CDF of the serial interval distribution of the disease of interest, that is the time period between the onset of symptoms in the primary and onset of symptoms in the secondary case, see @svensson2007.
Altogether, the paper by @nishiura_etal2016 uses that the probability $\pi_t = P(Z_t=1\>|\>D)$ can be computed as follows:
$$
\begin{align*}
\pi_t = 1 - \prod_{i=1}^n \sum_{o=0}^{\infty} f_{\text{offspring}}(o; R_0, k) \cdot \left[ F_{\text{serial}}(t-t_i) \right]^{o},
\end{align*}
$$
where $f_{\text{offspring}}$ denotes the PMF for the number of secondary cases one primary case induces. It is assumed that this distribution is negative binomial with expectation $R_0>0$ and clumping parameter $k>0$. In other words, $\operatorname{E}(O)=R_0$ and $\operatorname{Var}(O)=R_0 + R_0^2/k$. Once $\pi_t$ is below some pre-defined threshold $c$, say $c=0.05$, one would declare the outbreak to be over, if no new cases have been observed by time $t$. In other words:
$$
...
The the above $\pi_t$ equations given a predefined offspring distribution  (`doffspring`) are implemented as follows:
...
This allows us to re-calculate the results of @nishiura_etal2016:
...
We can embed estimation uncertainty originating from the estimation of $R_0$ and $k$ by adding an additional bootstrap step with values of $(\log R_0, \log k)'$ originating from the asymptotic normal distribution with expectation given to the MLE and variance-covariance matrix obtained from the observed Fisher information.
...
We can thus find the date where we would declare the outbreak
to be over as:
```{r}
c_threshold <- 0.05
...
Commenting on the above derivations done in @nishiura_etal2016 from a Bayesian viewpoint, it appears more natural to formulate the model directly in hierarchical terms:
...
Again, the simulation model is slightly conservative, because it does not take existing secondary cases in the data into account and samples $N_i$ new secondary cases for each observed case.
...
The present note introduced a statistical model based approach for declaring the end of a communicable disease outbreak such as MERS-Cov, Ebola, etc. If the considered outbreak has a different mode of transmission, e.g. foodborne or originates from a point-source, then different formulas apply, see e.g. @brookmeyer_you2006. Matters become even more complicated in case of vector borne disease outbreaks or in mixed transmission mode outbreaks as, e.g., STEC outbreaks. The adequate choice of $c$ as cut-off in the procedure would depend on the particular disease. As previously mentioned the method does not take under-reporting of cases as well as the possibility for asymptomatic cases into account. Furthermore, the present analysis assumed an estimate of the serial interval distribution was already available from other estimates. How to perform a reliable estimation of this quantity real-time during an outbreak (hence inducing right-truncation of long interval times) could be an interesting research question.

### Acknowledgements

I thank Hiroshi Nishiura for answering questions about his paper.
