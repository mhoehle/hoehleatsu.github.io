...
of $\beta = `r beta`$ means that the contact rate with a given individual is `r beta` contacts per day. The choice of $\gamma = `r gamma`$ corresponds to an average length of the infective period of 5 days. Furthermore, assuming  Altogether, this leads to an $R_0$ of `r R0`, which roughly corresponds to the $R_0$ of SARS-CoV-2.
...
Here we used the rather conservative estimate that $R_0$ can be reduced by `r scales::percent(reduction[1])` to `r R0*reduction[1]` for a few weeks, after the reduction is
