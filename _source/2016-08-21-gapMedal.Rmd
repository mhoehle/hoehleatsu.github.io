...
p3 <- ggplot(medals_gm, aes(x=GDPpc, y=FracPerPop, size = Population, color = Continent, frame = YearCity)) +
  ...
  geom_text(data=medals_gm, aes(x=GDPpc, y=FracPerPop, label=Nation), vjust=-0.7,show.legend=FALSE,size=ifelse(medals_gm$Nation %in% highlight_countries, 6, 4),alpha=ifelse(medals_gm$Nation %in% highlight_countries, 1,0.4), check_overlap=TRUE) +
  scale_x_log10() +
