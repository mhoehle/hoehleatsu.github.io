...
We now look at the country results for the summer games in the period 2000-2016.

```{r,FACET2000ANDBEYOND,echo=FALSE,warning=FALSE}
#Add city name for better visualization
olympic_city <- data.frame(Year=olympic_years, City=c("Rome","Tokyo", "Mexico City","Munich", "Montreal", "Moscow","Los Angeles", "Seoul", "Barcelona", "Atlanta", "Sydney", "Athens", "Beijing", "London","Rio de Janeiro"))
...
p2 <- ggplot(medals_gm, aes(x=GDPpc, y=Frac, size = Population, color = Continent, frame = YearCity)) +
  geom_point() +
  geom_text(data=medals_gm, aes(x=GDPpc, y=Frac, label=Nation), vjust=-0.7,show.legend=FALSE, size=1.4*(medals_gm$Population/1e6)^(1/4)+0.2) +
  scale_x_log10() +
  xlab("GDP per Capita") + ylab("Fraction of All Medals") +
