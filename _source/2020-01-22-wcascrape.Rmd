...
#How many started within last 5 years?
recent_senior_traj <- extended_results %>% inner_join(first_senior %>% select(personId) %>% collect(), by="personId") 

n_recent_senior_traj <- recent_senior_traj %>% summarise(n=n_distinct(personId)) %>% pull(n)

skill_bracket_breaks <- c(0, 30, 60, 90, 120)
recent_senior_traj <- recent_senior_traj %>% mutate(skill_bracket= cut(average_first_competition/100, skill_bracket_breaks))
```
