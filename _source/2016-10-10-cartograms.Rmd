...
Using the fantastic `rgdal`, `sp` and the `rgeos` CRAN packages, R can
be used as GIS. This allows for easy merging of these two data sources
together with an aggregation to the **Prognoseräume** level, which is
a slightly higher level of aggregation than the LORs (see GitHub code
for details).
...
```
With the functionality now being directly available through R allows
one to embedd operations in a full R pipeline. For example by gathering
a sequence of cartograms into an animated GIF file using the
`animation` package. The figure below shows a cartogram for the
population size for all available age groups. One observes that the 25-45
year old tend to live in the city centre, while the
95-110 year old seem to concentrate in the wealthy regions in the
south west.
...
saveGIF({
  for (i in 1:length(cartos)) {
    ...
    text(coordinates(pgrs_carto)[,1],coordinates(pgrs_carto)[,2],pgrs_name$EXTPGRNAME,cex=.45,col="darkgray")
  }
