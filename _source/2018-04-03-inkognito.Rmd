...
cat(paste0("![]({{ site.baseurl }}/",knitr::opts_chunk$get("fig.path"),"worksheet.jpg"),")")
```
</center>
...
```{r PLOTBELIEF}
id_names <- c("Lord\n Fiddlebottom","Madam\n Zsa Zsa", "Colonel Bubble", "Agent X")
d <- data.frame(prob=c(p_marginal(belief, what="I"),p_marginal(prior, "I")),
                what=factor(c(id_names,id_names), levels=id_names),
                type=factor(c(rep("Posterior",4),rep("Prior",4)),levels=c("Prior","Posterior")))
ggplot(d, aes(what, prob)) + geom_col() + theme_bw() +
  xlab("Identity") + ylab("Probability") +
