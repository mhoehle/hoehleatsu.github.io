...
spatstat::spatstat.options(npixel=250)
...
summariZe <- function(sim, print=TRUE) {
  ...
  summary <- sim %>% filter(!is.na(Type)) %>% group_by(Type) %>%
    summarise(p_out=mean(numbermiss>0),
              numbermiss=mean(numbermiss), p_miss = mean(missingfrac), `1-p_miss`=1-p_miss,
              numberobserved=mean(numberobserved), numberunobserved=mean(numberunobserved), totalnumber=mean(numberobserved+numberunobserved),area_zone=mean(arearegion)) %>%
