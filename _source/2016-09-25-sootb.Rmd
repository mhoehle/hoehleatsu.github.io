...
n_retweets <- zombieattack_updated$tw %>% filter(retweet_status_id == "780037067183157248") %>% summarise(n()) %>% unlist()
...
```{r,ZOMBIEUPDATE-POISSONCUSUM,echo=FALSE,message=FALSE,results='hide'}
#Use Poisson CUSUM with 
#  \log(\mu_{t,0}) = \beta_0 vs.
#  \log(\mu_{t,1}) = \beta_0 + \kappa
zombieattack_updated[["sts"]] <- glrnb(zombieattack_updated$ts, control=list(range = 9:nrow(zombieattack_updated$ts),mu0=list(S=0,trend=FALSE),c.ARL=1.9,ret="cases"))
