...
`r sprintf("%.1f%%",n_sex["m"]/sum(n_sex)*100)`. One caveat with the Berlin statistic is
...
for (distr in tolower(names(distrMap))) {
  ...
  text(loc[distr,1],loc[distr,2],topMale %$% firstname, col=palette["m"],pos=1,offset=0.3,font=2)
  text(loc[distr,1],loc[distr,2],topFemale %$% firstname, col=palette["f"],pos=3,offset=0.3,font=2)
}
