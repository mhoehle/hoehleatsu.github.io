...
It is obvious that $\phi_n(1)=1/n$. When $n=`r n`$, the remaining probabilities can
...
ggplot(df, aes(x=i,y=phi)) + geom_line() + ylab(expression(phi(i,n))) +
...
In the example we thus actually manage to select the best flat! However, this is not always guaranteed: for example, if the best overall candidate is among the training sample (`r r`/`r n` chance for this to happen) we would end up with the last candidate flat no matter how good or bad it is. As stated above: the probability that the above decision strategy will pick the best candidate is $\phi_{`r n`}(`r r`)=`r sprintf("%.2f",phi(r,n))`$.
...
For small $n$ the optimal $r$ and corresponding probability of success can easily be computed numerically. However, for large $n$ the numerical precision as well as the computations become more tedious and hence interest is in finding a general asymptotic approximation as $n$ grows large: One can show that as $n$ gets large the optimal procedure is always to screen the first $1/e$ = `r sprintf("%.0f",1/exp(1)*100)`% and then select the first candidate better than the training sample [@gilbert_mosteller1966]. The asymptotic probability of success, i.e. finding the overall best candidate, when following the such a procedure is also about `r sprintf("%.0f",1/exp(1)*100)`%. Below we show a small table illustrating the precision of the asymptotic approximation.
...
plotFun <- function() {
  stopIdx <- which(y <= s)
  for (i in 1:stopIdx) {
    plot(seq_len(i),y[1:i],type="p",xlim=c(1,n),ylim=c(0,max(y)),xlab="Candidate no.",ylab="Relative rank",pch=20)
    lines(seq_len(n),s+0,type="S",col="red",lty=2)
  }
...
saveGIF(plotFun(),movie.name="animation-select.gif",ani.width=600,ani.height=400)
```
...
From the results it becomes clear that the expected rank optimizing function on average takes a little less time to decide. Furthermore, the expected rank is somewhat better than for the overall best decision strategy. We can also compare `sim["erank","rank"]`=`r sprintf("%.3f",sim["erank","rank"])`  against $c_0$=`r sprintf("%.3f",strategy_erank(n)$c[1])`.
...
```{r,cache=TRUE,echo=FALSE,fig.align='center',fig.width=4,fig.height=4}
