...
* an update of the analysis containing the 2020 numbers as of `r date_of_analysis`, i.e. up to week `r str_c(isoyear(prelim_destatis_date), "-W", isoweek(prelim_destatis_date))` which covers the days from `r (prelim_destatis_date - 6)` to `r prelim_destatis_date`. 
...
The [R-markdown source code](`r paste0("https://raw.githubusercontent.com/hoehleatsu/hoehleatsu.github.io/master/_source/",current_input())`) of this blog is available under a [GNU
...
oe_adj <- destatis_deaths_range %>% 
  ...
  mutate(weight = case_when( (Woche == 01) ~ 5/7,
                             (W
                             TRUE ~ 1))
 utate(weight = case_when( (Jahr == 2020 & Woche == 01) ~ 5/7,
                             (Jahr == 2020 & Woche == 53) ~ 4/7,
                             TRUE ~ 1)) %>% 
summary <- function(x) {
  x %>% 
