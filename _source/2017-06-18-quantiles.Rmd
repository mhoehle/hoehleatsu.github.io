---
layout: post
title: "Pay Attention to the Argument: Flint Water Crisis and Quantiles"
...
story from a statistical viewpoint. Essentially, interest is in
whether the 90th percentile in a sample of lead concentration
monitoring measurements is above or below a certain threshold. We
...
In recent article, @langkjaerbain2017 tells the story about the
...
increased as lead in the aging pipes leached into the water supply. This
...
Quantile estimation in this context has already been discussed in a recent blog entry
...
which originally caught my attention to the topic.

The
[Lead and Copper Rule](https://www.epa.gov/dwreginfo/lead-and-copper-rule)
by the United States Environmental Protection Agency regulates the
monitoring of drinking water in the US. The entire text of the rule is
...
<p>
We note the explicit duality in the CFR between *in more than 10%* and the 90%
quantile as stated in the text. However, we also note that it is not
clear how to proceed, if the number calculated in (c)(3)(ii) is not an
integer. Since the CFR itself only operates with sizes 100, 60, 40, 20, 10 and 5
this is not per-se a problem because 0.9 times the CFR sample sizes always
...
The data of the Flint monitoring are presented in the figure on p. 20
...
the argument that they originated from sites which are not covered by the sampling plan,  which is supposed to only contain private households at high
risk, i.e. with lead pipelines. As one can read in the article this
step is considered highly controversial, in particular because a consequence was that with
these two values removed the proportion of critical observations fell
below the 10% action threshold.  For this blog entry, we will, however, work
...
$n=`r n`$. If more than 10% of the sample is to be above 15 ppb, $\lfloor 0.1\cdot n +
1\rfloor$ of the samples need to be above 15 ppb. We shall denote this
...
survey of @hyndman_fan1996. Here, they describe the nine different
...
the sample, i.e. let $x_{(1)} < x_{(2)} < \cdots < x_{(n)}$ denote the
...
`type=1`. For our application $p$ would be `r p` and the 90% quantile
for the Flint data is
```{r,echo=TRUE}
...
by Professor [Christopher Gardiner](http://www.emich.edu/math/faculty/cgardiner.php) on how to calculate the 90% quantile
using a `type=5` argument for the Flint sample. However, the important
point of the above calculations is that this type is **of limited interest**, because the Lead and Copper Rule implicitly defines that one has to use the
...
is tricky. As a step towards fixing this problem, one could redefine
the greater and less than operators, respectively, to only compare up to
possible numerical precision:
...
correct value to take and the 90% quantile of the sample is estimated to be `r quantile(lead,type=1,probs=p)` ppb. This means that the 19 ppb vertical line in
the figure of @langkjaerbain2017 is a little misleading as this appears to be
...
matter so much which method to take. However, in other settings this might very well make a difference. So be careful with the type argument. As an example, the nine different types of
R's `quantile` function provide estimates for the 90% quantile in the range `r sprintf("%.2f",min(q90_all))`-`r sprintf("%.2f",max(q90_all))` for the Flint data (`type=7` is the default in R).

On another note, one can discuss if it is a good idea to rely on
`type=1` quantile estimator in a regulation, because it is well known
...
quantile - a topic, which has been [previously treated theoretically](http://staff.math.su.se/hoehle/blog/2016/10/23/quantileCI.html) in this blog.
It is worthwhile pointing out that the CFR does not know about
confidence intervals. Currently, **estimation uncertainty** is only treated implicitly by specifying sample size as a function of number of people served by the water system. If one takes the additional estimation uncertainty into
account the number of critical observations would need to be even higher than what is currently specified by the CFR. Whether we as water consumers would accept this additional "benefit of doubt" due to uncertainty treatment without modification of the threshold or the requested sample sizes needs careful debate.
