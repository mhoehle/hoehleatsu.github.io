...
Because week 53 also covers parts of 2021, the analysis is currently done only up to week 52 in order to better compare with the previous years (who did not have a week 53). This can be improved.
...
destatis_deaths <- destatis_deaths %>% 
  ...
  # Note that we delete week 53 for the data to have an easier comparison
  # This needs to be fixed in an improved version of the week 53 handling. (no time at the moment)
  mutate(`53` = NA)
...
#ggplot(rki_deaths, aes(x=date, y=Anzahl, color=agegrp10)) + geom_line()
...
We note the strong regional differences in the plot. As an example, the highest mortality in the 65+ age-group occurs in `r sprintf("2020-W%0.2d",bl_max$Woche)` in the federal state of `r bl_max$Bundesland`, where the poulation adjusted mortality is `r scales::percent(bl_max$oe_ratio - 1)` above the mean of 2016-2019. Note that for `r bl_max$Bundesland`, even the <65 age-group currently has a visible excess. The `r nrow(bl_top)` federal states with the highest excess mortality week in the 65+ age-group are:
```{r}
...
```{r BLALLCAUSEANDCOVID, warning=FALSE, message=FALSE}
# Load data and impute "<4" entries (i.e. values 1-3) as 1.5
covid_bl_deaths_long <- readxl::read_xlsx(path=dest_file, sheet="COVID_Todesfälle_BL") %>% 
  rename(`Anzahl`=`Anzahl verstorbene COVID-19 Fälle`) %>% 
  select(-SterbeZeit) %>% 
  mutate(Sterbejahr = 2020,
         Anzahl = as.numeric(ifelse(Anzahl == "<4", 1.5, Anzahl))) %>%   
mutate(Date=ISOweek::ISOweek2date(sprintf("%d-W%02d-1",Sterbejahr,Sterbewoche)),
         Woche = isoweek(Date), Jahr=isoyear(Date)) %>% 
  select(-Sterbejahr,-Sterbewoche) %>% 
   left_join(tt, by=c("Bundesland"="Bundesland_kurz")) %>% 
  rename(Bundesland_kurz=Bundesland, Bundesland=Bundesland_lang)

# Reduce all-cause mortality and populations to a non-age-stratified version
bl_destatis_deaths_noage <- bl_destatis_deaths %>% group_by(Jahr, Woche, Bundesland) %>% 
  summarise(Anzahl=sum(Anzahl), Population=sum(Population))

# Joined dataset of all cause deaths (Destatis) and COVID deaths (RKI)
bl_deaths_noage_long <- bl_destatis_deaths_noage %>% rename(Anzahl_AllCause = `Anzahl`)  %>% 
  left_join(covid_bl_deaths_long %>% rename(Anzahl_COVID = Anzahl) %>% select(-Bundesland_kurz, -Date), 
            by=c("Jahr", "Woche", "Bundesland")) %>% 
  pivot_longer( cols=c(Anzahl_AllCause, Anzahl_COVID), names_to="Todesart", values_to="Anzahl") %>% 
  mutate(Todesart = gsub("Anzahl_", "", Todesart), 
         Vorjahr = Jahr < 2020, inc = Anzahl / Population) 

# Compute the indirect standardization
bl_oe_noage <-   bl_deaths_noage_long %>% 
  group_by(Woche, Bundesland, Vorjahr, Todesart) %>% 
  summarise(mean_Anzahl= mean(Anzahl), mean_inc = mean(inc), mean_Population = mean(Population)) %>% 
  pivot_wider(id_cols=c(Woche, Bundesland), names_from=c(Vorjahr, Todesart), values_from=c(mean_Anzahl, mean_inc, mean_Population)) %>% 
  rename( observed_2020_AllCause = mean_Anzahl_FALSE_AllCause,
          observed_2020_COVID = mean_Anzahl_FALSE_COVID) %>% 
  # perform the indirect standardisation
  mutate( expected_20162019_AllCause = mean_Population_FALSE_AllCause * mean_inc_TRUE_AllCause,
          oe_ratio_AllCause = observed_2020_AllCause / expected_20162019_AllCause,
          oe_ratio_AllCauseMinusCOVID = (observed_2020_AllCause - observed_2020_COVID) / (expected_20162019_AllCause)) 
```

```{r PLOTBLALLCAUSEANDCOVID, warning=FALSE, message=FALSE}
# Palette for plotting
pal2 <- rev(brewer_pal(palette="Dark2")(3)[2:3]) #dont use the green

bl_oe_noage_long <- bl_oe_noage %>% select(Woche, Bundesland, oe_ratio_AllCause, oe_ratio_AllCauseMinusCOVID) %>% 
  pivot_longer( cols=c(-Woche, -Bundesland), names_to="Type", values_to="SMR") %>% 
  mutate(Type = case_when( Type == "oe_ratio_AllCause" ~ "2020 All-Cause", Type == "oe_ratio_AllCauseMinusCOVID" ~ "2020 All-cause minus COVID-19", TRUE~Type))

ggplot(bl_oe_noage_long, aes(x=Woche, y=SMR, color=Type)) + geom_line() +
  scale_color_manual(values=c("2020 All-Cause"=pal2[1], "2020 All-cause minus COVID-19"=pal2[2], name="Type:")) +
  facet_wrap(~ Bundesland) +
  geom_hline(yintercept=1, lty=2, col="darkgray") +
  xlab("Week") +
  ylab("SMR") +
