...
#prepare_pop_data <- function(age_groups) 
...
covid_deaths_long <- covid_deaths %>% 
  ...
  left_join(pop_de_0030 %>% filter(Jahr==2020), by=c("Woche"="Woche", "age_group"="agegrp10")) %>% 
  #mutate(inc = number / Population * 1e5) %>% 
  filter(number >= 0) %>% 
  group_by(Date, age_group) %>% 
  # Recalc count and incidence after the summary
  summarise( number = sum(number), Population = sum(Population)) %>% 
  mutate(inc = number / Population)
...
covid_deaths_long2 <- covid_deaths_long %>% 
  mutate(
         ...
         rename(COVID = number, Bevölkerung=Population)
...
deaths_ts %>% group_by(Woche) %>% summarise(COVID = sum(COVID, na.rm=TRUE)) %>% tail()
