...
# # Number of initial sticks left in the game 
# n <- 8
...
# strategy <- tibble(state=states, value=U_vec, strategy=pi, prob=1/2*U_vec+1/2) %>% print(n=Inf)
# strategy <- strategy %>% 
...
# strategy
# 
# strategy %>% print(n=Inf)
# strategy %>% filter(!forced_move) %>% print(n=Inf)
