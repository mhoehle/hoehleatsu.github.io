...
(epid.nb, ##<< Number of outbreaks to be generated.
 ...
 family, ##<< Distribution type for the new cases, either "poisson", "negbin" or "empirical". (added by hoehle)
 negbin.size=NULL, ##<< Over-dispersion parameter, if family is set to "negbin".
 offspring.pmf=NULL, ##<< PMF of the offspring distribution starting with 0.
  ...
  
{
  ...
  if (family=="empirical" & is.null(offspring.pmf)) { #added by hoehle
    stop("No empirical offspring distribution defined.")
  }
  GT <- GT$GT
  #R0.orig <- R0
  ...
  for (n in 1:epid.nb) {
    ...
    for(t in 1:epid.length) { 
      ...
      if (family %in% c("poisson", "negbin","empirical")) {
        if (family=="poisson") {
          ...
          new <- sample(0:(length(offspring.pmf)-1), replace=TRUE, size=sim.epid[t], prob=offspring.pmf)
        }
        newd <-  rmultinom(1, sum(new), GT)[,1]
      }
      if (family=="pointmass") {
        newd <- R0*GT*sim.epid[t]
      }
