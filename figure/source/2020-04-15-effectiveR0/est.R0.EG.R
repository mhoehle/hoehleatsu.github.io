...
( epid, ##<< object containing epidemic curve data. see Details.
  ...
  reg.met=c("poisson","linear"), ##<< Regression method used. Default is "poisson" (for GLM), but can be forced to "linear".
...

{
  ...
  if (checked == FALSE) {
    parameters <- R0::integrity.checks(epid, t, GT, begin, end, date.first.obs, time.step, AR=NULL, S0=NULL, methods="EG")
  ...
  # Method 2 == Poisson regression
	...
	R = as.numeric(R0:::R.from.r(r,GT))

  ##details<< CI is computed from the 1/M(-r) formula using bounds on r from the Poisson regression.
	R.inf = as.numeric(R0:::R.from.r(confint[1],GT))
	R.sup = as.numeric(R0:::R.from.r(confint[2],GT))
